...
function MovieId() {
    ...
    const fetchTvInfo = async () => {
      try {
        const response = await axios.get(
          `https://api.themoviedb.org/3/tv/${id}?api_key=543affbe47ead4f79984688d45815285`
        );
  ...
  return (
    <section className=" h-[982px] flex">
      <div className="w-[20%]  rounded-r-[50px] border border-gray-600">
        ...
        <div className="flex items-center mt-[150px]">
          ...
          <p className="font-Poppins text-[#666] text-[20px] font-[600] leading-normal">Log Out</p>
        </div>
      </div>
      <div className="w-[80%]">
        <div className="w-full h-[550px] p-3 rounded-3xl">
          <img
            className="h-[550px] object-cover w-full rounded-3xl"
            src={`https://image.tmdb.org/t/p/original/${tvInfo.backdrop_path}`}
            alt=""
          />
        </div>
      </div>
